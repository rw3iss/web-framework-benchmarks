...
TNode<StackTraceFrame> Cast_StackTraceFrame_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_obj, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block0.is_used()) {
    ca_.Bind(&block0);
    compiler::CodeAssemblerLabel label1(&ca_);
    tmp0 = DownCastForTorqueClass_StackTraceFrame_0(state_, TNode<HeapObject>{p_obj}, &label1);
    ca_.Goto(&block3);
    if (label1.is_used()) {
      ca_.Bind(&label1);
      ca_.Goto(&block4);
  ...
  if (block3.is_used()) {
    ...
    ca_.Bind(&block5);
  return TNode<StackTraceFrame>{tmp0};
}
...
TNode<StackTraceFrame> DownCastForTorqueClass_StackTraceFrame_0(compiler::CodeAssemblerState* state_, TNode<HeapObject> p_o, compiler::CodeAssemblerLabel* label_CastError) {
  ...
  if (block3.is_used()) {
    ca_.Bind(&block3);
    if ((CodeStubAssembler(state_).ClassHasMapConstant<StackTraceFrame>())) {
      ca_.Goto(&block6);
    } else {
      ca_.Goto(&block7);
    }
  ...
  if (block6.is_used()) {
    ca_.Bind(&block6);
    tmp2 = CodeStubAssembler(state_).GetClassMapConstant<StackTraceFrame>();
    tmp3 = CodeStubAssembler(state_).TaggedNotEqual(TNode<HeapObject>{tmp1}, TNode<HeapObject>{tmp2});
    ca_.Branch(tmp3, &block9, std::vector<Node*>{}, &block10, std::vector<Node*>{});
  }
  ...
  if (block1.is_used()) {
    ...
    ca_.Bind(&block15);
  return TNode<StackTraceFrame>{tmp20};
}
