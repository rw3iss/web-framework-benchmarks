...
TF_BUILTIN(GetTemplateObject, CodeStubAssembler) {
  ...
  USE(parameter0);
  TNode<SharedFunctionInfo> parameter1 = UncheckedCast<SharedFunctionInfo>(Parameter(Descriptor::ParameterIndex<0>()));
  USE(parameter1);
  TNode<TemplateObjectDescription> parameter2 = UncheckedCast<TemplateObjectDescription>(Parameter(Descriptor::ParameterIndex<1>()));
  USE(parameter2);
...
TF_BUILTIN(BytecodeBudgetInterruptFromCode, CodeStubAssembler) {
  ...
  TNode<Context> parameter0 = UncheckedCast<Context>(Parameter(Descriptor::kContext));
  USE(parameter0);
  TNode<FeedbackCell> parameter1 = UncheckedCast<FeedbackCell>(Parameter(Descriptor::ParameterIndex<0>()));
  USE(parameter1);
  compiler::CodeAssemblerParameterizedLabel<> block0(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
    ca_.Goto(&block0);
