...
USE(parameter2);
  ...
  if (block0.is_used()) {
    ...
    tmp5 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltin(Builtins::kIterableToListWithSymbolLookup, parameter0, tmp4));
    tmp6 = CodeStubAssembler(state_).ErrorsStringConstant();
    tmp7 = CodeStubAssembler(state_).SmiConstant(PropertyAttributes::DONT_ENUM);
    CodeStubAssembler(state_).CallRuntime(Runtime::kSetOwnPropertyIgnoreAttributes, parameter0, tmp2, tmp6, tmp5, tmp7);
