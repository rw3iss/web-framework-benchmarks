...
#define DEFINE_TORQUE_GENERATED_SHARED_FUNCTION_INFO_FLAGS2() \
  ...
  using MayHaveCachedCodeBit = base::BitField<bool, 2, 1, uint8_t>; \
  enum Flag { \
    kNone = 0, \
    kClassScopeHasPrivateBrand = 1 << 0, \
    kHasStaticPrivateMethodsOrAccessors = 1 << 1, \
    kMayHaveCachedCode = 1 << 2, \
  }; \
  using Flags = base::Flags<Flag>; \
  static constexpr int kFlagCount = 3; \
...
#define DEFINE_TORQUE_GENERATED_SOURCE_TEXT_MODULE_FLAGS() \
  ...
  using AsyncEvaluatingBit = base::BitField<bool, 1, 1, uint32_t>; \
  enum Flag { \
    kNone = 0, \
    kAsync = 1 << 0, \
    kAsyncEvaluating = 1 << 1, \
  }; \
  using Flags = base::Flags<Flag>; \
  static constexpr int kFlagCount = 2; \

#define DEFINE_TORQUE_GENERATED_STACK_FRAME_INFO_FLAGS() \
  using IsEvalBit = base::BitField<bool, 0, 1, uint32_t>; \
  using IsConstructorBit = base::BitField<bool, 1, 1, uint32_t>; \
  using IsWasmBit = base::BitField<bool, 2, 1, uint32_t>; \
  using IsAsmJsWasmBit = base::BitField<bool, 3, 1, uint32_t>; \
  using IsUserJavaScriptBit = base::BitField<bool, 4, 1, uint32_t>; \
  using IsToplevelBit = base::BitField<bool, 5, 1, uint32_t>; \
  using IsAsyncBit = base::BitField<bool, 6, 1, uint32_t>; \
  using IsPromiseAllBit = base::BitField<bool, 7, 1, uint32_t>; \
  using IsPromiseAnyBit = base::BitField<bool, 8, 1, uint32_t>; \
  enum Flag { \
    kNone = 0, \
    kIsEval = 1 << 0, \
    kIsConstructor = 1 << 1, \
...
#define DEFINE_TORQUE_GENERATED_FUNCTION_TEMPLATE_INFO_FLAGS() \
  ...
  using DoNotCacheBit = base::BitField<bool, 4, 1, uint32_t>; \
  using AcceptAnyReceiverBit = base::BitField<bool, 5, 1, uint32_t>; \
  enum Flag { \
    kNone = 0, \
    kUndetectable = 1 << 0, \
    kNeedsAccessCheck = 1 << 1, \
    kReadOnlyPrototype = 1 << 2, \
    kRemovePrototype = 1 << 3, \
    kDoNotCache = 1 << 4, \
    kAcceptAnyReceiver = 1 << 5, \
  }; \
  using Flags = base::Flags<Flag>; \
  static constexpr int kFlagCount = 6; \
...
#define DEFINE_TORQUE_GENERATED_OBJECT_TEMPLATE_INFO_FLAGS() \
  ...
  using EmbedderFieldCountBits = base::BitField<int32_t, 1, 29, uint32_t>; \
